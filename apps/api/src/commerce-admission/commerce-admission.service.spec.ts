...
describe('CommerceAdmissionService', () => {
  ...
  it('caches an explicit organization denial for exactly 30 seconds', async () => {
    ...
    await expect(admission.admitCreateBox('org-1', { cpu: 1, gpu: 0, mem: 1, disk: 10 })).rejects.toMatchObject({
      status: 402,
      response: { statusCode: 402, message: 'INSUFFICIENT_AVAILABLE_CREDIT', error: 'Payment Required' },
    })
    await expect(admission.admitCreateBox('org-1', { cpu: 1, gpu: 0, mem: 1, disk: 1 })).rejects.toMatchObject({
      status: 402,
    })
  ...
  it('does not cache technical failures and fails open', async () => {
    ...
    await expect(admission.admitCreateBox('org-1', { cpu: 1, gpu: 0, mem: 1, disk: 10 })).resolves.toBeNull()
    await expect(admission.admitCreateBox('org-1', { cpu: 1, gpu: 0, mem: 1, disk: 10 })).resolves.toBeNull()
    expect(post).toHaveBeenCalledTimes(2)
    expect(Logger.prototype.warn).toHaveBeenCalledWith(expect.stringContaining('allowing create'))
  })

  it('accepts a valid response and sends the fixed contract with a 500ms timeout', async () => {
    const admission = service()
    post.mockResolvedValue({ data: allowed })

    await expect(admission.admitCreateBox('org-1', { cpu: 2, gpu: 1, mem: 4, disk: 20 })).resolves.toEqual({
